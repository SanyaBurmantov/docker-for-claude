...
### 1) Купить аккаунт Claude на плати [https://plati.market/itm/24-7-claude-ai-5-0-pro-max-code-subscription-to-your-account-1-month-auto/5652859](https://plati.market/itm/24-7-claude-ai-5-0-fable-pro-max-code-subscription-new-account-1-month-auto/5885369) или любом другом, автору никто не приплачивает за рекламу
...
Тот же Claude Code, но запущенный за локальным прокси [Headroom](https://github.com/headroomlabs-ai/headroom): он стоит между CLI и `api.anthropic.com` и сжимает объёмный ввод (вывод инструментов, логи, содержимое файлов) до отправки в модель, экономя токены на каждом ходе.
...
`claude-hr` (`claude-container/scripts/claude-hr`) при первом запуске поднимает `headroom proxy` (порт 8787), выставляет `ANTHROPIC_BASE_URL` и передаёт все аргументы в `claude`; прокси общий на контейнер. Установлен базовый `headroom-ai` (без ML-экстры `[all]`). Диагностика: `headroom doctor`, `curl -s 127.0.0.1:8787/stats`. Подробнее — в `ai-platform/README.md`.
