...
6. На вкладке **Git** — commit, rollback, переключение веток, pull/push. Общий селектор **AI** в шапке проекта выбирает Claude, Codex или Gemini одновременно для проектного чата, сообщения коммита, дневного отчёта, review и объяснения diff
...
Для одноразовых AI-действий и чата провайдеры автоматически резервируют друг друга: **Claude → Codex → Gemini → бесплатные модели OpenCode**. Цепочка начинается с выбранного провайдера, поэтому выбор Codex или Gemini не расходует Claude. Последний список можно переопределить через `OPENCODE_FREE_MODELS` в `.env` (несколько моделей — через запятую).
